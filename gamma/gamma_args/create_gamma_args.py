...
class GammaArgs:
    ...
    def create_args_par_S1_grd(self):
        ...
        print("------- Creating necessary Variables --------")
        # Create Import Dir
        idir = "C:/Users/felix/OneDrive/Desktop/gammaGUI/data/S1A_IW_SLC__1SDV_20170731T163002_20170731T163029_017717_01DAC5_4796"
        # WorkEnv[1]
        #print(idir)
        ...
        outname_MLI = list()
        outname_GRD_par = list()
        outname_GRD = list()
        # optional parameterkk
        eflg = ""  # 0 is default(FCOMPLEX), 1 (SCOMPLEX)
        rps = ""  # Scale Factro for FCOMPLEX -> SCOMPLEX, default HH,VV (dB): 60.000 , VH,HV: 70.000
        noise_pwr = ""
        # print("Import dir: " + idir)
        # print("Export dir: " + odir)
        # print("Root:")
    ...
    def create_args_par_S1_GRD(self):
        """
                This is the Function to Create all needed Arguments in the correct Order for the par_S1_SLC Module.
                Corresponding Window: gammaGUIv2/gui_windows/QSubWindow_S1_GRD.py where the Input comes from
                Therefore:
                    - the WorkEnv.xml,GammaCommands.xml is read and the WorkEnv and Import Directory are extracted
                    - The Import Directory contains: the Extracted *zip Files in the *.SAFE Format
                    - For every Folder (*.SAFE) in this ImportDIR a loop is executed:
        ...
        for i in range(len(outname_base)):
            print("Hallo")
            print((" ".join(["mkdir", outname_base[i],
                             " ".join(["&& mv", outname_SLC_par[i][0], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][0], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][0], '"$_"']),
                             " ".join(["&& mv", outname_SLC_par[i][1], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][1], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][1], '"$_"']),
                             " ".join(["&& mv", outname_SLC_par[i][2], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][2], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][2], '"$_"']),
                             " ".join(["&& mv", outname_SLC_par[i][3], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][3], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][3], '"$_"']),
                             " ".join(["&& mv", outname_SLC_par[i][4], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][4], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][4], '"$_"']),
                             " ".join(["&& mv", outname_SLC_par[i][5], '"$_"']),
                             " ".join(["&& mv", outname_SLC[i][5], '"$_"']),
                             " ".join(["&& mv", outname_SLC_tops_par[i][5], '"$_"'])])))
            copy_args_tmp = (
                " ".join(["mkdir", outname_base[i],  # TODO Find out behaviour on Server possibly Change Linux Syntax
                          " ".join(["&& mv", outname_SLC_par[i][0], '"$_"']),
                          " ".join(["&& mv", outname_SLC[i][0], '"$_"']),
                          " ".join(["&& mv", outname_SLC_tops_par[i][0], '"$_"']),
                          " ".join(["&& mv", outname_SLC_par[i][1], '"$_"']),
